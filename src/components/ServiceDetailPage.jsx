...
function OrbitVisual({ service }) {
  ...
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[510px]">
      <motion.div
        className="absolute inset-[12%] rounded-full border border-[#8bd3ff]/25"
        animate={{ rotate: 360 }}
        transition={{ duration: 24, repeat: Infinity, ease: 'linear' }}
      />
      <motion.div
        className="absolute inset-[25%] rounded-full border border-dashed border-[#8bd3ff]/20"
        animate={{ rotate: -360 }}
        transition={{ duration: 34, repeat: Infinity, ease: 'linear' }}
      />
      <div className="absolute inset-[5%] rounded-full bg-[#153b7d]/40 blur-3xl" />
      <motion.div
        className="absolute left-1/2 top-1/2 flex h-[42%] w-[42%] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[30%] border border-[#8bd3ff]/40 bg-[#0b245e]/90 shadow-[0_0_70px_rgba(104,197,255,0.28)] backdrop-blur-xl"
        animate={{ scale: [1, 1.04, 1], rotate: [0, 2, 0] }}
        transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
      >
        <div className="flex h-[72%] w-[72%] flex-col items-center justify-center rounded-[24%] border border-white/10 bg-[#07184d]/90">
          <Icon className="mb-3 h-10 w-10" style={{ color: service.accent }} strokeWidth={1.4} />
          <span className="text-center text-[9px] font-semibold tracking-[0.2em] text-white/60">EDRISYNC</span>
          <span className="mt-1 text-center text-[8px] tracking-[0.14em] text-white/35">SERVICE CORE</span>
        </div>
      </motion.div>
      <motion.div
        className="absolute left-[7%] top-[20%] flex items-center gap-2 rounded-full border border-white/15 bg-[#123477]/90 px-3 py-2 text-[10px] text-white/80 shadow-lg backdrop-blur-md"
        animate={{ y: [0, -8, 0] }}
      ...
      <motion.div
        className="absolute bottom-[18%] right-[4%] flex items-center gap-2 rounded-full border border-white/15 bg-[#123477]/90 px-3 py-2 text-[10px] text-white/80 shadow-lg backdrop-blur-md"
        animate={{ y: [0, 8, 0] }}
      ...
      {[0, 1, 2].map((index) => (
        <motion.span
          ...
          style={{ backgroundColor: service.accent, left: `${[19, 74, 84][index]}%`, top: `${[72, 18, 68][index]}%`, boxShadow: `0 0 18px 5px ${service.accent}88` }}
      ...
      <main id="service-start">
        <section className="mx-auto grid max-w-7xl gap-12 px-6 py-20 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:py-28">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative overflow-hidden rounded-[28px] bg-[#102d72] shadow-[0_24px_70px_rgba(6,17,83,0.18)]">
            ...
            <div className="absolute bottom-7 left-7 max-w-xs text-white"><Sparkles className="mb-3 h-5 w-5" style={{ color: service.accent }} /><p className="text-lg font-medium leading-snug">One connected program. Clearer outcomes.</p></div>
          </motion.div>
        </section>

        <section className="bg-white px-6 py-20 lg:py-24">
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 max-w-xl"><span className="text-xs font-semibold uppercase tracking-[0.28em]" style={{ color: service.color }}>What we deliver</span><h2 className="mt-4 text-3xl font-light sm:text-4xl">Capability built around your next move.</h2></div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {service.capabilities.map((capability, index) => (
                <motion.div key={capability} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className="group relative min-h-[168px] overflow-hidden rounded-2xl border border-slate-200 bg-[#f8faff] p-6 transition-colors hover:border-[#8bd3ff]">
        ...
        <section className="mx-auto max-w-7xl px-6 py-20 lg:py-24">
          <div className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr]">
            <div><span className="text-xs font-semibold uppercase tracking-[0.28em]" style={{ color: service.color }}>The outcome</span><h2 className="mt-4 text-3xl font-light sm:text-4xl">Built to make progress visible.</h2></div>
            <div className="grid gap-4 sm:grid-cols-2">{service.outcomes.map((outcome, index) => <motion.div key={outcome} initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className="flex gap-3 border-t border-slate-200 pt-4"><Check className="mt-0.5 h-5 w-5 shrink-0" style={{ color: service.color }} /><p className="text-sm leading-relaxed text-slate-600">{outcome}</p></motion.div>)}</div>
          </div>
        </section>
        ...
        <section className="px-6 py-20 text-center lg:py-28"><Network className="mx-auto mb-5 h-8 w-8" style={{ color: service.color }} /><h2 className="mx-auto max-w-2xl text-3xl font-light sm:text-4xl">Ready to make this service work for your organization?</h2><p className="mx-auto mt-5 max-w-xl text-slate-600">Let’s talk through where you are, what is changing, and the next practical step.</p><button onClick={onHome} className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#153b7d] px-6 py-3 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5">Start a conversation <ArrowRight className="h-4 w-4" /></button></section>
      </main>
    </div>
  );
}
...
function GRCLayout({ service, onServices, onHome }) {
  return (
    <div className="min-h-screen bg-white text-[#061153]">
