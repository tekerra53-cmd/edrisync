...
function CaseStudyVisual({ project }) {
  ...
  return (
    <div className="flex h-full items-center justify-center bg-[#040720]">
      <div className="rounded-xl border border-white/20 bg-white/10 p-6 text-white backdrop-blur-sm">
        <Icon className="h-6 w-6 text-[#B1B6CE]" />
...
export default function CaseStudiesPage({ onHome }) {
  ...
  return (
    <section className="min-h-screen bg-[#ffffff] px-5 pb-24 pt-32 sm:px-6 sm:pb-28">
      <div className="mx-auto max-w-7xl">
        <button onClick={onHome} className="inline-flex items-center gap-2 text-sm text-slate-500 transition-colors hover:text-[#040720]"><ArrowLeft className="h-4 w-4" /> Back to home</button>
        <div className="mt-16 max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#087fd1]">Case studies</p>
          <h1 className="mt-4 text-5xl font-light leading-[1.04] text-[#040720] sm:text-6xl">More work. More measurable outcomes.</h1>
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-2">
          {caseStudies.map((project, index) => {
            const Icon = project.icon;
            return (
              <motion.article key={project.title} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: index * 0.08 }} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(4,7,32,0.05)] transition-shadow duration-300 hover:shadow-[0_18px_40px_rgba(4,7,32,0.12)]">
                ...
                <div className="p-7">
                  ...
                  <h2 className="mt-5 text-2xl font-light text-[#040720]">{project.title}</h2>
        ...
        <div className="mt-20 rounded-2xl bg-[#040720] px-7 py-12 text-center text-white sm:px-12"><h2 className="text-3xl font-light">Have a challenge worth solving?</h2><p className="mx-auto mt-4 max-w-xl text-white/65">Let’s talk about the outcome your organization needs next.</p><button onClick={onHome} className="mt-7 inline-flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-semibold transition-colors hover:bg-[#B1B6CE]" style={{ backgroundColor: '#B1B6CE', color: '#040720' }}>Start a conversation <ArrowUpRight className="h-4 w-4" /></button></div>
      </div>
    </section>
  );
}
