...
const perspectiveSeed = [
  {
    id: 'perspective-1', title: 'Security-first Microsoft environment', client: 'Enterprise technology organization',
    ...
    author: 'Sarah Chen',
    role: 'Head of Information Security', status: 'Published', image: '',
  },
  {
    ...
    role: 'Operations Director', client: 'Growing professional services firm', status: 'Published', image: '',
  },
  {
    ...
    author: 'Aisha Patel',
    role: 'Finance Director', client: 'Regulated financial services group', status: 'Published', image: '',
  },
  {
    ...
    author: 'David Mensah',
    role: 'Chief Operating Officer', client: 'Regional logistics company', status: 'Published', image: '',
  },
  {
    ...
    role: 'People and Technology Lead', client: 'Multi-site healthcare provider', status: 'Published', image: '',
  },
  {
    ...
    author: 'Emeka Okafor',
    role: 'Transformation Manager', client: 'Professional services organization', status: 'Published', image: '',
  },
];

export const defaultClientPerspectives = perspectiveSeed.map((item, index) => ({
  ...item,
  id: item.id || `perspective-${index + 1}`,
  title: item.title || item.client,
}));

export default function Testimonials({ sectionRef }) {
  const [testimonials, setTestimonials] = useState(defaultClientPerspectives);
  const [activeIndex, setActiveIndex] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const active = testimonials[activeIndex];

  useEffect(() => { getManagedContent('client_perspectives', defaultClientPerspectives).then((items) => { const published = items.filter((item) => item.status !== 'Draft'); setTestimonials(published.length ? published : defaultClientPerspectives); setActiveIndex(0); }); }, []);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-14 lg:grid-cols-[0.72fr_1.28fr] lg:items-center lg:gap-20">
          <motion.div initial={{ opacity: 0, x: -18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.65 }}>
            ...
            <h2 className="mt-5 max-w-md text-4xl font-light leading-[1.08] text-[#000741] sm:text-5xl">Client success stories that prove the impact.</h2>
            ...
            <div className="mt-10 flex items-center gap-3">
              <button onClick={() => move(-1)} aria-label="Previous testimonial" className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-300 shadow-sm transition-colors hover:border-[#B1B6CE]" style={{ backgroundColor: '#ffffff', color: '#000741' }}><ArrowLeft className="h-4 w-4" /></button>
              <button onClick={() => move(1)} aria-label="Next testimonial" className="flex h-11 w-11 items-center justify-center rounded-full shadow-sm transition-colors hover:bg-[#000741]" style={{ backgroundColor: '#000741', color: '#ffffff' }}><ArrowRight className="h-4 w-4" /></button>
          ...
          <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_18px_45px_rgba(4,7,32,0.07)] sm:p-10" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} onFocus={() => setIsPaused(true)} onBlur={() => setIsPaused(false)}>
            ...
            <AnimatePresence mode="wait">
              <motion.figure key={active.author} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ duration: 0.35 }}>
                ...
                <blockquote className="max-w-4xl text-xl font-light leading-relaxed text-[#000741] sm:text-2xl">“{active.quote}”</blockquote>
                <figcaption className="mt-9">
                  <p className="text-base font-semibold text-[#000741]">{active.author}</p>
            ...
            <div className="mt-12 flex items-center justify-between gap-5" role="tablist" aria-label="Testimonials">
              ...
              {testimonials.map((testimonial, index) => (
                <button key={testimonial.author} onClick={() => setActiveIndex(index)} role="tab" aria-selected={activeIndex === index} aria-label={`Show testimonial ${index + 1}`} className={`h-2.5 rounded-full transition-all ${activeIndex === index ? 'w-7 bg-[#000741]' : 'w-2.5 bg-slate-300 hover:bg-[#B1B6CE]'}`} />
              ))}
              </div>
              <div className="h-1.5 w-24 overflow-hidden rounded-full bg-slate-100"><div key={activeIndex} className="testimonial-progress h-full rounded-full bg-[#4ADE80]" /></div>
            </div>
          </div>
        </div>
      </div>
